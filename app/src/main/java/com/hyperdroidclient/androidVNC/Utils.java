...
public class Utils {
	...
	public static boolean checkStatus(Context context){
		...
		databaseReference.child("VirtualMachine").child(new SharedPreferenceManager(context).getVMKey()).child("Status").setValue("0");
		new SharedPreferenceManager(context).saveVMStatus(false);
		return false;
	}
}
